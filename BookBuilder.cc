...
std::vector<BookBuilder::Order, Allocator<BookBuilder::Order>> BookBuilder::getBestBids(const Symbol &symbol) {
    std::vector<Order, Allocator<Order>> bestBids;
...
std::vector<BookBuilder::Order, Allocator<BookBuilder::Order>> BookBuilder::getBestOffers(const Symbol &symbol) {
    std::vector<Order, Allocator<Order>> bestOffers;
    for (const Order& order : orders[symbol]) {
        if (order.side == Ask) {
